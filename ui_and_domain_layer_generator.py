...
for root, dirs, files in os.walk(ui_and_domain_template_path):
    os.makedirs(where_to_generate_dir + (root.replace('${mf_name}', mf_snake_case)).replace(ui_and_domain_template_path, ''))
    for _file in files:
        file_path = os.path.join(root, _file)
        new_file_path = where_to_generate_dir + (file_path.replace('${mf_name}', mf_snake_case)).replace(ui_and_domain_template_path, '')
