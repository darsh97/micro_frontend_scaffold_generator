# Micro Frontend Data Layer Scaffold Generator
...
data_template_path = 'template/data/'

for root, dirs, files in os.walk(data_template_path):
    os.makedirs(where_to_generate_dir + (root.replace('${mf_snake_case}', mf_snake_case)).replace(data_template_path, ''))
    for _file in files:
        file_path = os.path.join(root, _file)
        new_file_path = where_to_generate_dir + (file_path.replace('${mf_snake_case}', mf_snake_case)).replace(data_template_path, '')
